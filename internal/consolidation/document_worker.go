...
func (wp *WorkerPool) processDocumentJob(ctx context.Context, workerID int, job DocumentJob) {
	log.Printf("[Document Worker %d] Starting processing for document %s (%s)", workerID, job.DocumentID, job.SourceType)

	// 1. Update status to Processing
	if err := wp.Store.UpdateDocumentStatus(ctx, job.DocumentID, document.StatusProcessing, ""); err != nil {
	...
	case document.SourceGoogleDocs:
	...
	// Sanitize text and metadata strings to be valid UTF-8, replacing invalid sequences with empty strings.
	// This prevents gRPC/protobuf marshalling errors during embedding or fact extraction requests.
	...
	if err := wp.Store.UpdateDocumentStatus(ctx, job.DocumentID, document.StatusCompleted, ""); err != nil {
...
func (wp *WorkerPool) failJob(ctx context.Context, workerID int, docID uuid.UUID, err error) {
	...
	_ = wp.Store.UpdateDocumentStatus(ctx, docID, document.StatusFailed, err.Error())
}

func (wp *WorkerPool) extractCognitiveGraph(ctx context.Context, workerID int, docID uuid.UUID, chunks []document.DocumentChunk) {
	...
	for _, chunk := range chunks {
		...
		for _, ext := range facts {
			...
			if isSingularAttribute(ext.Attribute) {
				activeFacts, err := wp.Store.SearchHybrid(bgCtx, &memory.MemorySearchQuery{
