...
func RunEntityResolution(ctx context.Context, store ports.MemoryRepository) error {
	...
	for i := 0; i < len(entities); i++ {
		...
		for j := i + 1; j < len(entities); j++ {
			...
			if len(entA.Embedding) > 0 && len(entB.Embedding) > 0 {
				var err error
				vecSim, err = entity.CosineSimilarity(entA.Embedding, entB.Embedding)
				if err != nil {
					vecSim = 0.0
				}
			}
...
// CalculateJaroWinkler calculates the Jaro-Winkler similarity distance between two strings
func CalculateJaroWinkler(s1, s2 string) float64 {
	...
	// Winkler prefix scale (standard scaling factor p=0.1, max prefix 4 chars)
	prefixLimit := 4
	prefixLen := 0
	for i := 0; i < int(math.Min(float64(prefixLimit), math.Min(float64(len1), float64(len2)))); i++ {
		if s1[i] == s2[i] {
			prefixLen++
		} else {
			break
