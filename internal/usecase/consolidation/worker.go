...
type WorkerPool struct {
	JobQueue      chan entity.InteractionLog
	DocumentQueue chan entity.DocumentJob
...
func NewWorkerPool(store ports.MemoryRepository, chatMemory ports.ChatMemoryRepository, llm ports.LLMService, cache ports.SemanticCache, queueSize int, maxWorkers int) *WorkerPool {
	return &WorkerPool{
		JobQueue:      make(chan entity.InteractionLog, queueSize),
		DocumentQueue: make(chan entity.DocumentJob, queueSize),
		Store:         store,
...
// QueueInteraction implements ports.ConsolidationService
func (wp *WorkerPool) QueueInteraction(log entity.InteractionLog) {
	wp.JobQueue <- log
}

// QueueDocument implements ports.ConsolidationService
func (wp *WorkerPool) QueueDocument(job entity.DocumentJob) {
	wp.DocumentQueue <- job
}
...
func (wp *WorkerPool) processJob(ctx context.Context, workerID int, job entity.InteractionLog) {
	log.Printf("[Worker %d] Ingesting message from session %s for fact & relation extraction", workerID, job.SessionID)

	ctx = entity.WithAgentOwner(ctx, job.EntityID)
	...
	var (
		extractedFacts []entity.ExtractedFact
		factsErr       error
		extractedRels  []entity.ExtractedRelation
		relsErr        error
	...
	if len(extractedFacts) > 0 {
		activeFacts, err := wp.Store.SearchHybrid(ctx, &entity.MemorySearchQuery{
		...
		if err != nil {
			...
			activeMap := make(map[string]entity.Fact)
			...
			for _, ext := range extractedFacts {
				...
				wgFacts.Add(1)
				go func(ext entity.ExtractedFact) {
					defer wgFacts.Done()
	...
	if len(extractedRels) > 0 {
		...
		for _, r := range extractedRels {
			wgRels.Add(1)
			go func(rel entity.ExtractedRelation) {
				defer wgRels.Done()
				...
				if targetName == "user" || targetName == "subject" || targetName == "john" || targetName == "speaker" || targetName == senderName {
					...
					if err == nil {
						_ = wp.Store.InsertFact(ctx, &entity.Fact{
							ID:              uuid.New(),
							EntityID:        targetID,
							Attribute:       "name",
...
func isSingularAttribute(attr string) bool {
	...
	if strings.Contains(attrLower, "hobb") ||
		strings.Contains(attrLower, "interest") ||
		strings.Contains(attrLower, "injury") ||
		strings.Contains(attrLower, "hospital") ||
		strings.Contains(attrLower, "allergy") ||
		strings.Contains(attrLower, "medication") {
		return false
	}
