...
func NewConflictValidator(llm ports.LLMService, store ports.MemoryRepository) *ConflictValidator {
...
// It returns the validation result, and if a conflict is found, it attempts to match it to an existing entity.Fact.
func (cv *ConflictValidator) CheckConflict(ctx context.Context, entityID uuid.UUID, candidateFact entity.ExtractedFact, activeFacts []entity.Fact) (*ConflictResult, *entity.Fact, error) {
	...
	factMap := make(map[string]entity.Fact)
