...
func RunCognitiveReflection(ctx context.Context, store ports.MemoryRepository, llm ports.LLMService) error {
	...
	entityMap := make(map[uuid.UUID]entity.EntityNode)
	...
	for _, ent := range entities {
		// Fetch active facts
		facts, err := store.SearchHybrid(ctx, &entity.MemorySearchQuery{
			TargetEntity: ent.ID,
		...
		factsByOwner := make(map[uuid.UUID][]entity.Fact)
		for _, f := range facts {
			factsByOwner[f.AgentOwner] = append(factsByOwner[f.AgentOwner], f)
		}

		// Group relations by AgentOwner
		relsByOwner := make(map[uuid.UUID][]entity.Relation)
		for _, r := range rels {
		...
		for ownerID, ownerFacts := range factsByOwner {
			...
			var reflectionFacts []entity.Fact
			...
			var reflectionRels []entity.Relation
...
If no valid logical deductions can be made, output an empty JSON array [].`, ent.Name, factsStr.String(), relsStr.String())
			...
			for _, ins := range insights {
				...
				ownerCtx := entity.WithAgentOwner(ctx, ownerID)
