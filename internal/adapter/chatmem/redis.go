...
// RedisChatMemory implements ChatMemoryRepository using a Redis connection pool.
...
func (r *RedisChatMemory) AppendMessage(ctx context.Context, sessionID string, msg entity.ChatMessage) error {
	conn := r.pool.Get()
	defer conn.Close()
...
func (r *RedisChatMemory) GetSessionHistory(ctx context.Context, sessionID string, limit int) ([]entity.ChatMessage, error) {
	conn := r.pool.Get()
	defer conn.Close()
	...
	history := make([]entity.ChatMessage, len(values))
